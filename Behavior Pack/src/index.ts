...
const activeTexts: Map<Player, number> = new Map();
 ...
 */
function stopAnimation(target: Player): void {
 ...
 */
export function animateText(target: Player, text: string, subtitle: string | null, speed: number, mode: 'actionbar' | 'title', waitTime: number = 20): void {
    ...
    const intervalId = system.runInterval(() => {
        let letterProcess = (target.getDynamicProperty('letterProcess') as number) || 0;
 ...
 * @param speed - The update interval in ticks.
 */
function animateSubtitle(target: Player, title: string, subtitle: string, speed: number): void {
    stopAnimation(target);
 ...
 */
function applyCharacterProgression(text: string, letterCount: number): string {
    let displayText = '';
